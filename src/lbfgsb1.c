 ...
 *  modified to be standalone of part of the optim
 */
...
static void active1(int, double *, double *, int *, double *, int *,
		   int, int *, int *, int *);
static void bmv1(int, double *, double *, int *, double *, double *, int *);
static void cauchy1(int, double *, double *,
		   double *, int *, double *, int *, int *,
		   double *, double *, double *, int, double *,
		   double *, double *, double *, double *, int * ,
		   int *, double *, double *, double *, double *,
		   int *, int, double *, int *, double *);
static void cmprlb1(int, int, double *,
		   double *, double *, double *, double *,
		   double *, double *, double *, double *, int *,
		   double *, int *, int *, int *, int *,
		   int *);
static void dcsrch1(double *, double *, double *,
		   double, double, double,
		   double, double,
		   char *, int *, double *);
static void dcstep1(double *, double *,
		   double *, double *, double *, double *,
		   double *, double *, double *, int *, double *,
		   double *);
#ifdef NOT_USING_DBL_EPSILON
 static double dpmeps1(void);
#endif
static void errclb1(int, int, double,
		   double *, double *, int *, char *, int *, int *);
static void formk1(int, int *, int *, int *, int *, int *, int *,
		  int *, double *, double *, int, double *,
		  double *, double *, double *, int *, int *, int *);
static void formt1(int, double *, double *,
		  double *, int *, double *, int *);
static void freev1(int, int *, int *,
		  int *, int *, int *, int *, int *, int *,
		  int *, int, int *);
static void hpsolb1(int, double *, int *, int);
static void lnsrlb1(int, double *, double *,
		   int *, double *, double *, double *, double *,
		   double *, double *, double *, double *,
		   double *, double *, double *, double *,
		   ...
		   char *, int *, double *);
static void mainlb1(int, int, double *,
...
static void matupd1(int, int, double *, double *, double *,
		   double *, double *, double *, int *, int *,
		   int *, int *, double *, double *, double *,
		   double *, double *);
static void projgr1(int, double *, double *,
		   int *, double *, double *, double *);
static void subsm1(int, int, int *, int *, double *, double *,
		  int *, double *, double *, double *, double *,
		  double *, int *, int *, int *, double *,
		  double *, int, int *);

static void prn1lb1(int n, int m, double *l, double *u, double *x,
		   int iprint, double epsmch);
static void prn2lb1(int n, double *x, double *f, double *g, int iprint,
		   int iter, int nfgv, int nact, double sbgnrm,
		   int nint, char *word, int iword, int iback,
		   double stp, double xstep);
static void prn3lb1(int n, double *x, double *f, char *task, int iprint,
...
void setulb1(int n, int m, double *x, double *l, double *u, int *nbd,
...
*/
    ...
    lt = isave[15];
    lwa = isave[16];
    mainlb1(n, m, x, l, u, nbd, f, g, factr, pgtol,
...
static void mainlb1(int n, int m, double *x,
...
/*	  Check the input arguments for errors. */
	errclb1(n, m, factr, &l[1], &u[1], &nbd[1], task, &info, &k);
	if (strncmp(task, "ERROR", 5) == 0) {
	    prn3lb1(n, x+1, f, task, iprint, info,
		   iter, nfgv, nintol, nskip, nact, sbgnrm,
		   nint, word, iback, stp, xstep, k);
	    return;
	}

	prn1lb1(n, m, l+1, u+1, x+1, iprint, epsmch);

/*	  Initialize iwhere & project x onto the feasible set. */
	active1(n, &l[1], &u[1], &nbd[1], &x[1], &iwhere[1], iprint, &prjctd,
		&cnstnd, &boxed);
...
/*     Compute the infinity norm of the (-) projected gradient. */
    projgr1(n, &l[1], &u[1], &nbd[1], &x[1], &g[1], &sbgnrm);
...
/* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
    timer1(&cpu1);
    cauchy1(n, &x[1], &l[1], &u[1], &nbd[1], &g[1], &indx2[1], &iwhere[1], &t[
...
/*     find the index set of free and active variables at the GCP. */
    freev1(n, &nfree, &indx[1], &nenter, &ileave, &indx2[1], &iwhere[1], &
...
/*		       [ 0  I] */
    if (wrk) {
	formk1(n, &nfree, &indx[1], &nenter, &ileave, &indx2[1], &iupdat, &
...
/*						     from 'cauchy'). */
    cmprlb1(n, m, &x[1], &g[1], &ws[ws_offset], &wy[wy_offset], &sy[sy_offset]
...
/*	 call the direct method. */
    subsm1(n, m, &nfree, &indx[1], &l[1], &u[1], &nbd[1], &z[1], &r[1], &
...
/* L40: */
    ...
    timer1(&cpu1);
L666:
    lnsrlb1(n, &l[1], &u[1], &nbd[1], &x[1], f, &fold, &gd, &gdold, &g[1], &
...
/*	  Print iteration information. */
	prn2lb1(n, x+1, f, g+1, iprint, iter, nfgv, nact,
...
/*     Update matrices WS and WY and form the middle matrix in B. */
    matupd1(n, m, &ws[ws_offset], &wy[wy_offset], &sy[sy_offset], &ss[
...
/*	     J' stored in the upper triangular of wt. */
    formt1(m, &wt[wt_offset], &sy[sy_offset], &ss[ss_offset], &col, &theta, &
	    info);
    if (info != 0) {
/*	    nonpositive definiteness in Cholesky factorization; */
/*	    refresh the lbfgs memory and restart the iteration. */
...
static void active1(int n, double *l, double *u,
...
static void bmv1(int m, double *sy, double *wt,
...
static void cauchy1(int n, double *x, double *l, double *u, int *nbd,
...
/*     Initialize derivative f2. */
    ...
    f2_org__ = f2;
    if (*col > 0) {
	bmv1(m, &sy[sy_offset], &wt[wt_offset], col, &p[1], &v[1], info);
	if (*info != 0) {
	    return;
	}
...
/*	   cost of heapsort is avoided. */
	...
	hpsolb1(nleft, &t[1], &iorder[1], iter - 2);
	tj = t[nleft];
	ibp = iorder[nleft];
    }
    dt = tj - tj0;
...
/*	     compute (wbp)Mc, (wbp)Mp, and (wbp)M(wbp)'. */
	bmv1(m, &sy[sy_offset], &wt[wt_offset], col, &wbp[1], &v[1], info);
	if (*info != 0) {
	    return;
	}
...
static void cmprlb1(int n, int m, double *x,
...
*/
	...
	bmv1(m, &sy[sy_offset], &wt[wt_offset], col,
	    &wa[(m << 1) + 1], &wa[1], info);
	if (*info != 0) {
	    *info = -8;
	    return;
...
static void errclb1(int n, int m, double factr, double *l, double *u,
...
static void formk1(int n, int *nsub, int *ind, int * nenter, int *ileave,
...
static void formt1(int m, double *wt, double *sy, double *ss,
...
static void freev1(int n, int *nfree, int *indx,
...
static void hpsolb1(int n, double *t, int *iorder, int iheap)
{
/*	************
...
static void lnsrlb1(int n, double *l, double *u,
...
static void matupd1(int n, int m, double *ws,
...
static void projgr1(int n, double *l, double *u,
...
static void subsm1(int n, int m, int *nsub, int *ind,
...
static void dcsrch1(double *f, double *g, double *stp,
...
/*	  Call dcstep to update stx, sty, and to compute the new step. */
	dcstep1(&stx, &fxm, &gxm, &sty, &fym, &gym, stp, &fm, &gm, &brackt, &
		stmin, &stmax);
...
/*	 Call dcstep to update stx, sty, and to compute the new step. */
	dcstep1(&stx, &fx, &gx, &sty, &fy, &gy, stp, f, g, &brackt, &stmin, &
		stmax);
    }
...
static void dcstep1(double *stx, double *fx, double *dx,
...
/**********************************************************************
* functions required for lbfgsb1
**********************************************************************/
...
static double fminfn1(int n, double *p, void *ex, SEXP x1)
{
    ...
    int i;
    double val;
    pOptStruct pOS = (pOptStruct) ex;
    PROTECT_INDEX ipx;

    //PROTECT(x = allocVector(REALSXP, n));
	PROTECT(x = coerceVector(x1, REALSXP));
    if(!isNull(pOS->names)) setAttrib(x, R_NamesSymbol, pOS->names);
    for (i = 0; i < n; i++) {
	if (!R_FINITE(p[i])) error(_("non-finite value supplied by optim"));
	REAL(x)[i] = p[i] * (pOS->parscale[i]);
    }
    SETCADR(pOS->R_fcall, x);
    PROTECT_WITH_INDEX(s = eval(pOS->R_fcall, pOS->R_env), &ipx);
    REPROTECT(s = coerceVector(s, REALSXP), ipx);
    if (LENGTH(s) != 1)
	error(_("objective function in optim evaluates to length %d not 1"),
	      LENGTH(s));
    val = REAL(s)[0]/(pOS->fnscale);
    UNPROTECT(2);
    return val;
}
...
static void fmingr1(int n, double *p, double *df, void *ex, SEXP x1)
{
    ...
    if (!isNull(pOS->R_gcall)) { /* analytical derivatives */
	//PROTECT(x = allocVector(REALSXP, n));
	PROTECT(x = coerceVector(x1, REALSXP));
	if(!isNull(pOS->names)) setAttrib(x, R_NamesSymbol, pOS->names);
	for (i = 0; i < n; i++) {
	...
	SETCADR(pOS->R_gcall, x);
	PROTECT_WITH_INDEX(s = eval(pOS->R_gcall, pOS->R_env), &ipx);
	REPROTECT(s = coerceVector(s, REALSXP), ipx);
	if(LENGTH(s) != n)
	    error(_("gradient in optim evaluated to length %d not %d"),
		  LENGTH(s), n);
	for (i = 0; i < n; i++)
	    df[i] = REAL(s)[i] * (pOS->parscale[i])/(pOS->fnscale);
	UNPROTECT(2);
    } else { /* numerical derivatives */
	//PROTECT(x = allocVector(REALSXP, n));
	PROTECT(x = coerceVector(x1, REALSXP));
	setAttrib(x, R_NamesSymbol, pOS->names);
	for (i = 0; i < n; i++) REAL(x)[i] = p[i] * (pOS->parscale[i]);
	SETCADR(pOS->R_fcall, x);
	if(pOS->usebounds == 0) {
	    for (i = 0; i < n; i++) {
		eps = pOS->ndeps[i];
		REAL(x)[i] = (p[i] + eps) * (pOS->parscale[i]);
		SETCADR(pOS->R_fcall, x);
		PROTECT_WITH_INDEX(s = eval(pOS->R_fcall, pOS->R_env), &ipx);
		REPROTECT(s = coerceVector(s, REALSXP), ipx);
		val1 = REAL(s)[0]/(pOS->fnscale);
		REAL(x)[i] = (p[i] - eps) * (pOS->parscale[i]);
		SETCADR(pOS->R_fcall, x);
		REPROTECT(s = eval(pOS->R_fcall, pOS->R_env), ipx);
		REPROTECT(s = coerceVector(s, REALSXP), ipx);
		val2 = REAL(s)[0]/(pOS->fnscale);
...
#define DO_df_x							\
	    ...
	    for (i = 0; i < n; i++) {
		epsused = eps = pOS->ndeps[i];
		tmp = p[i] + eps;
		if (tmp > pOS->upper[i]) {
		    tmp = pOS->upper[i];
		    epsused = tmp - p[i] ;
		}
		REAL(x)[i] = tmp * (pOS->parscale[i]);
		SETCADR(pOS->R_fcall, x);
		PROTECT_WITH_INDEX(s = eval(pOS->R_fcall, pOS->R_env), &ipx);
		REPROTECT(s = coerceVector(s, REALSXP), ipx);
		val1 = REAL(s)[0]/(pOS->fnscale);
...
/**********************************************************************
 * lbfgsb1 is a twick of an original lbfgsb in optim.c from the 
 *  R : A Computer Language for Statistical Data Analysis
 ...
 **********************************************************************/
/**********************************************************************
 * the twick is: allocate memory wa and iwa outside of the lbfgsb1
 * for the very simple reason: if we compute large number of models with
 * the same dimensions, it is better to do allocation once
 * add such arguments to lbfgsb1:
 * double wa, has dimension of 2*m*n+4*n+11*m*m+8*m
 * int iwa, has dimension of 3*n
 **********************************************************************/
...
{
    char task[60];
    double f, dsave[29];//,*g,*wa;
    int tr = -1, iter = 0, isave[44], lsave[4];//, *iwa;

    if(n == 0) { /* not handled in setulb */
	*fncount = 1;
	*grcount = 0;
	*Fmin = fminfn1(n, u, ex,x1);
    ...
    *fail = 0;
	...
	setulb1(n, m, x, l, u, nbd, &f, g, factr, &pgtol, wa, iwa, task,
